<template>
    <div :class="[$style.TheNotification, {[$style._allNotif]: allNotifications}]">
        <div :class="$style.list">

            <div v-for="(item, index) in notificationSliced"
                 :key="index"
                 :class="[$style.item, $style[item.type]]"
            >
                <div :class="$style.iconWrap">
                    <div :class="$style.icon">
                        <svg width="24"
                             height="24"
                             viewBox="0 0 24 24"
                             fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 22C6.477 22 2 17.523 2 12C2 6.477 6.477 2 12 2C17.523 2 22 6.477 22 12C22 17.523 17.523 22 12 22ZM8 13C8 14.0609 8.42143 15.0783 9.17157 15.8284C9.92172 16.5786 10.9391 17 12 17C13.0609 17 14.0783 16.5786 14.8284 15.8284C15.5786 15.0783 16 14.0609 16 13H8ZM8 11C8.39782 11 8.77936 10.842 9.06066 10.5607C9.34196 10.2794 9.5 9.89782 9.5 9.5C9.5 9.10218 9.34196 8.72064 9.06066 8.43934C8.77936 8.15804 8.39782 8 8 8C7.60218 8 7.22064 8.15804 6.93934 8.43934C6.65804 8.72064 6.5 9.10218 6.5 9.5C6.5 9.89782 6.65804 10.2794 6.93934 10.5607C7.22064 10.842 7.60218 11 8 11ZM16 11C16.3978 11 16.7794 10.842 17.0607 10.5607C17.342 10.2794 17.5 9.89782 17.5 9.5C17.5 9.10218 17.342 8.72064 17.0607 8.43934C16.7794 8.15804 16.3978 8 16 8C15.6022 8 15.2206 8.15804 14.9393 8.43934C14.658 8.72064 14.5 9.10218 14.5 9.5C14.5 9.89782 14.658 10.2794 14.9393 10.5607C15.2206 10.842 15.6022 11 16 11Z"
                                  fill="white" />
                        </svg>
                    </div>
                </div>

                <div :class="$style.text">
                    {{ item.text }}
                </div>

            </div>

            <div v-if="showButton"
                 :class="$style.btn"
                 @click.stop="showAllNotifications"
            >
                Посмотреть все
            </div>

        </div>
    </div>
</template>

<script>
    export default {
        name: 'TheNotification',
        props: {
            notifications: {
                type: Array,
                default: () => []
            }
        },

        data() {
            return {
                showButton: {
                    type: Boolean,
                    default: true
                },

                allNotifications: {
                    type: Boolean,
                    default: false
                }
            };
        },

        computed: {
            notificationSliced() {
                if (this.showButton) {
                    return this.notifications.slice(0, 3);
                }

                return this.notifications;
            }
        },

        created() {
            this.allNotifications = false;
            this.showButton = this.notifications.length > 3;
        },

        methods: {
            showAllNotifications() {
                this.showButton = false;
                this.allNotifications = true;
            }
        }
    };
</script>

<style lang="scss" module>
    .TheNotification {
        width: 33.3rem;
        max-height: 36rem;
        border-radius: 20px;
        background: rgba(255, 255, 255, 0.01);
        box-shadow: 0 4px 60px #111925;
        backdrop-filter: blur(100px);
        transition: all $transition;
        overflow: hidden;

        &::-webkit-scrollbar {
            display: none;
        }

        &._allNotif {
            max-height: 50rem;
            overflow: auto;
        }
    }

    .list {
        display: flex;
        flex-direction: column;
        width: 100%;
        height: 100%;
    }

    .item {
        padding: 2.6rem 3.5rem;
        border-bottom: 1px solid rgba(#fff, 0.04);
        display: flex;

        &:first-child {
            padding-top: 3.4rem;
        }

        &:last-child {
            border-bottom: 0;
        }

        &.bonus {
            .iconWrap {
                background-color: #ff00c7;
            }
        }
    }

    .iconWrap {
        display: flex;
        align-items: center;
        justify-content: center;
        flex-shrink: 0;
        width: 3.6rem;
        height: 3.6rem;
        margin-right: 1.4rem;
        border-radius: 10px;
    }

    .icon {
        width: 2.4rem;
        height: 2.4rem;
    }

    .text {
        font-weight: 500;
        font-size: 1.6rem;
        line-height: 1.9rem;
        color: #747980;
    }

    .btn {
        margin: 3.2rem 0;
        font-size: 1.6rem;
        font-weight: 500;
        opacity: .4;
        text-align: center;
        cursor: pointer;
        transition: opacity $transition;

        &:hover {
            opacity: 1;
        }
    }

</style>
